...
/**
 * Old implementation of ModelAnimationsReader that
 ...
 */
@Deprecated
public class BedrockModelAnimationsReader {
    ...
    private List<KeyFrame> readKeyFrames(JsonElement element) {
        if (element == null) {
            ...
            return new ArrayList<>(Collections.singletonList(new KeyFrame(
                    0,
                    Serialization.getVector3FloatFromJson(element)
            )));
        } else {
            ...
            for (Map.Entry<String, JsonElement> frameEntry : json.entrySet()) {
                ...
                Vector3Float value = Serialization.getVector3FloatFromJson(frameEntry.getValue());
